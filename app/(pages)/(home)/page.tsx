	...
	const onSubmit = useCallback(
		async (
			...
			try {
				...
				const recommendedGameNames = games.map((game: Game) => game.name);

				// wait for all game descriptions to be fetched before updating state
				await Promise.all(
					recommendedGameNames.map(async (game: string) => {
						const description = await getGameDescription(gameInput, game);
						games.find((_game: Game) => _game.name === game).description =
							description;
					}),
				);
	...
	return (
		<Provider store={store}>
			<main className="main w-full flex flex-col items-center pb-20 bg-[#F5F5F5]">
				<div
					className={`flex flex-col justify-center items-center ${pageReady ? "mt-20" : "h-screen"}`}
				>
					{/* logo */}
					<Image
						priority
